...
module.exports = class UserRepository extends BaseRepository {
    ...
    async findAndUpdate({ query, updateQuery, findType, select = null }) {
        if (findType === 'email') {
            return UserModel.findOneAndUpdate({ email: query }, updateQuery, { new: true }).select(select).session(this.session);
        } else {
            // if more fields are needed, add else if condition
            return UserModel.findByIdAndUpdate(query, updateQuery, { new: true }).select(select).session(this.session);
        }
    ...
    async createOrUpdateById(id, body) {
        if (id) {
            return UserModel.findByIdAndUpdate(id, { ...body }, { new: true }).session(this._session)
        } else {
            return (await new UserModel(body).save({ session: this._session })).generateToken()
        }
    }
